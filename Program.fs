...
let rec Treemap1 tree floatvalue index =

    match tree with
    | None -> None
    | Some n ->
        Some {
            D = floatvalue n.D (float index)
            Left = Treemap1 n.Left floatvalue index
            Right = Treemap1 n.Right floatvalue index
        }
...
and IndexInput tree =
    ...
    | (true, index) -> 
        let treerezult = Treemap1 tree floatvalue index
        printfn "Дерево: "
        Print1 treerezult ""
    | _ -> 
...
let rec Treefoldsum tree maps acc =
    match tree with
    | None -> acc
    | Some n -> 
        let len = n.D.Length
        let LSum = Treefoldsum n.Left maps acc
        let RSum = Treefoldsum n.Right maps acc
        let T = maps len LSum RSum 
        T
...
let rec input2 () =
    ...
    | "1" ->
        ...
        | Some tree -> 
            ...
            Print2 (Some tree) ""
            let acc = 0
            let sumstring = Treefoldsum (Some tree) maps acc
    ...
    | "2" -> 
        ...
        | (true, n) when n > 0 -> 
            ...
            Print2 tree ""
            let acc = 0
            let sumstring = Treefoldsum tree maps acc
            printfn ""
            printfn "Сумма длин всех строк в дереве: %d" sumstring
        | _ -> 
